...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            'total',
            type=int,
            help='Indicates the number of resumes to be created',
        )
        parser.add_argument(
            '--filepath',
            type=str,
    ...
    def handle(self, *args, **kwargs):
        ...
        sample_df["job_title"] = (
            sample_df["Resume_str"]
            .apply(strip_to_lower, forget_last=2)
        )
        ...
        # Assign resumes
        resumes = []
        for i, resume_row in tqdm(
            sample_df.iterrows(),
            desc="Assigning resumes",
            unit="resume",
            file=stdout,
        ):
            user = choice(users)
